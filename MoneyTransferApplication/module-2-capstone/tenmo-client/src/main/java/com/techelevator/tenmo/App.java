...
App {
    ...
    private void viewPendingRequests() {
        ...
        for (Transfer transfer : currentUserTransfers) {
            ...
            String usernameTo = accountService.getUsernameByAccountId(transfer.getAccountTo());
            BigDecimal amount = transfer.getAmount();
            if (transferTypeId == 1 && transferStatusId != 2 && transferStatusId != 3) {
    ...
    private void sendBucks() {
        ...
        System.out.println("");    // second form of confirmation to make sure user is sending bucks to the correct person
...
// method for the user to request buck from other tenmo users if all parameters are met (valid user account, ID, balance)
    private void requestBucks () {
        ...
        String receiverUsername = accountService.getUsernameByAccountId(accountService.getAccountByUserId(receiver).getAccountId());
        BigDecimal amount = consoleService.promptForBigDecimal("Please enter the amount you are requesting from " + receiverUsername + "?: ");
        System.out.println();
        System.out.println("Are you sure you would like to request " + currency.format(amount) + " from " + receiverUsername + "?");
        String userConfirmation = consoleService.promptForString("Y/N: ");
        System.out.println();
